    ...
    public static class MCPToolRegistry
    {
        ...
        private static void EnsureInitialized()
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    Debug.LogWarning($"[LocalMCP] Failed to scan assembly {name}: {e.Message}");
                }
            }

            Debug.Log($"[LocalMCP] Registered {_tools.Count} tools: {string.Join(", ", _tools.Keys)}");
        }
        ...
        private static void ScanAssembly(Assembly assembly)
        {
            foreach (var type in assembly.GetTypes())
            {
                foreach (var method in type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
                {
                    ...
                    {
                        Debug.LogWarning($"[LocalMCP] Tool {toolAttr.Name} has invalid signature. Expected (JObject args)");
                        continue;
                    }
        ...
        public static object InvokeTool(string name, JObject args)
        {
            ...
            {
                Debug.LogWarning($"[LocalMCP] Slow tool: {name} took {elapsedMs}ms (threshold: {SlowToolThresholdMs}ms)");
            }

            // Check for timeout (tool completed but took too long - log warning)
            if (elapsedMs > timeoutMs)
            {
                Debug.LogError($"[LocalMCP] Tool {name} exceeded timeout: {elapsedMs}ms > {timeoutMs}ms");
