...
namespace LocalMCP
{
    ...
    public static class AutoRefreshWatcher
    {
        ...
        {
            get => EditorPrefs.GetBool("LocalMCP_AutoRefresh", true);
            set
            {
                EditorPrefs.SetBool("LocalMCP_AutoRefresh", value);
        ...
        private static void StartWatching()
        {
            ...
            try
            {
                ...
                Debug.Log("[LocalMCP] AutoRefreshWatcher started - external file changes will auto-trigger compilation");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[LocalMCP] Failed to start AutoRefreshWatcher: {e.Message}");
            }
        }
        ...
        private static void StopWatching()
        {
            if (_watcher != null)
            {
                ...
                _watcher.Dispose();
                _watcher = null;
                Debug.Log("[LocalMCP] AutoRefreshWatcher stopped");
            }
        }
        ...
        {
            Debug.LogWarning($"[LocalMCP] FileSystemWatcher error: {e.GetException().Message}");
        ...
        private static void OnUpdate()
        {
            ...
            if (scriptCount > 0)
            {
                Debug.Log($"[LocalMCP] Auto-refreshing: {scriptCount} script(s) changed externally");
            }
