...
namespace UnityMCP
{
    ...
    public static class MCPBootstrap
    {
        ...
        {
            get => SessionState.GetBool("UnityMCP_ShownStartupMessage", false);
            set => SessionState.SetBool("UnityMCP_ShownStartupMessage", value);
        }
        ...
        private static void OnFirstFrame()
        {
            ...
            if (!EditorPrefs.HasKey("UnityMCP_AutoStart"))
            {
                EditorPrefs.SetBool("UnityMCP_AutoStart", false);
                EditorPrefs.SetInt("UnityMCP_Port", 8090);
                Debug.Log("[UnityMCP] First-time setup: Server disabled by default. Use Tools > MCP > Control Panel to start, or run 'claude mcp add --transport http unity http://localhost:8090/mcp -s project'");
            }

            // Ensure AutoRefreshWatcher is enabled by default
            if (!EditorPrefs.HasKey("UnityMCP_AutoRefresh"))
            {
                EditorPrefs.SetBool("UnityMCP_AutoRefresh", true);
                Debug.Log("[UnityMCP] First-time setup: Auto-refresh watcher enabled");
            }

            // Start server if it should be running but isn't
            if (EditorPrefs.GetBool("UnityMCP_AutoStart", false) && !MCPServer.IsRunning)
            {
                if (!EditorApplication.isCompiling)
                {
                    EditorApplication.delayCall += () =>
                    {
                        if (!MCPServer.IsRunning)
                        {
                            ...
                            MCPServer.Start(EditorPrefs.GetInt("UnityMCP_Port", 8090));
                        }
                    };
                }
            }
        ...
        private static void ShowStartupStatus()
        {
            ...
            EditorApplication.delayCall += () =>
            {
                if (MCPServer.IsRunning)
                {
                    Debug.Log($"[UnityMCP] Unified server ready - Port {MCPServer.Port}, {MCPServer.ToolCount} tools, AutoRefresh: {(AutoRefreshWatcher.IsEnabled ? "ON" : "OFF")}");
                }
            };
        }
        ...
        public static void ResetToDefaults()
        {
            ...
            Debug.Log("[UnityMCP] Reset complete - Server disabled by default.");
        }
        ...
        public static void EnableMCP()
        {
            ...
            Debug.Log($"[UnityMCP] Enabled - {MCPServer.ToolCount} tools now available. Run 'claude mcp add --transport http unity http://localhost:8090/mcp -s project' to connect Claude.");
        }
        ...
        public static void DisableMCP()
        {
            EditorPrefs.SetBool("UnityMCP_AutoStart", false);
            MCPServer.Stop();
            Debug.Log("[UnityMCP] Disabled. Run 'claude mcp remove unity -s project' to disconnect from Claude.");
        }
