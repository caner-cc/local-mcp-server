...
namespace UnityMCP
{
    ...
    public static class MCPServer
    {
        ...
        {
            Debug.LogWarning("[UnityMCP] Force stop initiated - clearing all pending requests");
            ...
            Debug.Log("[UnityMCP] Force stop completed. Server stopped and queue cleared.");
        }
        ...
        public static int ClearRequestQueue()
        {
            ...
            if (cleared > 0)
            {
                Debug.LogWarning($"[UnityMCP] Cleared {cleared} pending requests from queue");
            }
        ...
        static MCPServer()
        {
            ...
            {
                int port = EditorPrefs.GetInt("UnityMCP_Port", 8090);
                StartImmediate(port);
            }
        }
        ...
        private static void OnCompilationFinished(object obj)
        {
            ...
            else if (EditorPrefs.GetBool("UnityMCP_AutoStart", false))
            {
        ...
        private static void StartImmediate(int port)
        {
            ...
            foreach (int tryPort in portsToTry)
            {
                try
                {
                    ...
                    if (tryPort != port)
                    {
                        Debug.LogWarning($"[UnityMCP] Port {port} in use, started on fallback port {tryPort}");
                        EditorPrefs.SetInt("UnityMCP_Port", tryPort);
                    }
                    return; // Success!
                }
                catch (Exception e)
                {
                    ...
                    {
                        Debug.LogError($"[UnityMCP] Failed to start on any port (tried {string.Join(", ", portsToTry)}): {e.Message}");
                        _running = false;
                    }
                }
            }
        ...
        private static void TryInitializeToolRegistry()
        {
            ...
            catch (Exception e)
            {
                ...
                Debug.LogWarning($"[UnityMCP] Tool init deferred: {e.Message}");
            }
        }
        ...
        public static void Start(int port = 8090, bool silent = false)
        {
            if (IsRunning)
            {
                if (!silent) Debug.Log($"[UnityMCP] Already running on port {Port}");
                return;
            }
            ...
            if (!silent && IsRunning)
                Debug.Log($"[UnityMCP] Server started on http://localhost:{port}/");
        }
        ...
        private static void ListenLoop()
        {
            while (_running && _listener != null)
            {
                ...
                {
                    if (_running)
                        Debug.LogError($"[UnityMCP] Listener error: {e.Message}");
                }
            }
        }
        ...
        private static void ProcessPendingRequests()
        {
            ...
            while (processed < 10)
            {
                ...
                {
                    Debug.LogWarning($"[UnityMCP] Error handling request: {e.Message}");
        ...
        private static void HandleRequest(HttpListenerContext context)
        {
            ...
            {
                Debug.LogError($"[UnityMCP] Request error: {e}");
            ...
            switch (method)
            {
                case "initialize":
                    result = new
                    {
                        ...
                        serverInfo = new { name = "UnityMCP", version = "1.0.0" }
                    };
                    break;
        ...
        private static object HandleToolCall(JObject @params)
        {
            ...
            try
            {
                ...
                if (resultJson.Length > MaxResponseSizeBytes)
                {
                    ...
                    Debug.LogWarning($"[UnityMCP] Response for {toolName} truncated: {resultJson.Length} > {MaxResponseSizeBytes} bytes");
                }
